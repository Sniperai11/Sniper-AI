...
### 3.2 استراتيجية اختبارات الأمان المضمنة (Security Testing Strategy & DevSecOps)

لضمان خلو الأكواد البرمجية من الثغرات الأمنية (Vulnerabilities) قبل وصولها لبيئات التشغيل، تُطبق المنصة استراتيجية تدقيق آلي ثلاثية المستويات مدمجة بالكامل في ممر التطوير المستمر (CI/CD Pipeline) لمنع دمج الأكواد غير الآمنة:

1. **التحليل الأمني الساكن (Static Application Security Testing - SAST):**
   * **الأدوات المعتمدة:** استخدام أداة **Semgrep** لفحص قواعد الأمان المخصصة للكود، بالتكامل مع **SonarQube** لتحليل جودة البرمجيات ورصد العيوب الفنية (Code Smells).
   * **السياسة الأمنية:** يُمشط الكود للبحث عن استدعاءات خطيرة (مثل `eval` أو تشغيل أوامر نظام مباشرة عبر مدخلات ملوثة) وتخزين المفاتيح الصلبة (Hardcoded Secrets).
   * **معيار المنع (Gatekeeper Rule):** تُحظر عملية الدمج (Blocking Rule) تلقائياً في مستودع GitHub إذا تم اكتشاف أي ثغرة مصنفة بمستوى خطورة **حرج (Critical)** أو **عالٍ (High)**.

2. **التحليل الأمني الديناميكي (Dynamic Application Security Testing - DAST):**
   * **الأداة المعتمدة:** تشغيل حاوية فحص مؤتمتة لـ **OWASP ZAP (ZAP CLI)** تستهدف بيئة المعاينة الفورية المؤقتة (Preview/Staging Sandbox).
   * **السياسة الأمنية:** إرسال طلبات فحص نشطة وموجهة للتحقق من عيوب التحقق من الهوية (Authentication Flaws)، ثغرات تجاوز الصلاحيات (Broken Access Control)، وغياب ترويسات حماية أمان النقل (Missing Security Headers).

3. **فحص التبعيات والمكتبات الخارجية (Software Composition Analysis - SCA):**
   * **الأدوات المعتمدة:** تشغيل فحص الأمان المدمج لـ **npm audit** كخطوة أولية، تليها عملية تدقيق معمقة عبر أداة **Snyk CLI**.
   * **سياسة فحص التبعيات:** تُمنع المنصة دمج أي كود يستدعي مكتبة خارجية تحتوي على ثغرات أمنية ذات تقييم خطورة تجاوز **7.0** على مقياس خطورة الثغرات الموحد (CVSS v3)، ما لم يتم إدراج استثناء مبرر أمنياً وموافق عليه من رئيس الفريق الأمني، مع تفعيل تحديثات التبعيات التلقائية أسبوعياً لمعالجة المشاكل المكتشفة.
